...
def api_check_score():
    ...
    cursor.execute("SELECT highscore FROM users WHERE username = %s;",
                   (str(username),))  # Execute select statement
    ...
    if len(result) > 0:
        if result[0][0] < val:
            ...
            cursor.execute(
                "UPDATE users SET highscore = %s WHERE username = %s;", (str(val), str(username)))
            connection.commit()

    cursor.execute("SELECT highscore FROM users WHERE username = %s;",
                   (str(username),))  # Get current highscore
    result = cursor.fetchall()  # Store result of this statement
